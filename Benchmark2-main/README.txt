Il lavoro è suddiviso principalmente in 3 parti: 
- Prima pagina; (Benjamin)
- Seconda pagina; (Mihajlo-Nouha)
- Terza pagina; (Simone)
- footer, side bar (Nouha)

- Rendere la pagina responsive
- Fare le animazioni
- Implementare JS (principalmente per l'hamburger menu)
- Implementare funzione immagin full screen (tipo carosello a tutto schermo, quindi con la possibilità di cambiare immagini)
- Implementare Dark mode

Faremo un mix tra bootstrap e css vanilla. 